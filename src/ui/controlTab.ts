...
export function controlTab(model: PeepSimModel): WidgetCreator<FlexiblePosition>[] {
    const vis0 = modeVis(model, 0);
    const vis1 = modeVis(model, 1);
    const vis2 = modeVis(model, 2);
            ...
            content: [
                toggle({
                    ...
                    onChange: (pressed: boolean) => {
                        if (pressed) activateMoveTool(model);
                        else deactivateMoveTool(model);
                    },
                }),
                horizontal({
                    ...
                    content: [
                        ...
                        vertical([
                            horizontal([
                                button({
                                    image: SPR_DIR_NW, width: "45px", height: "29px",
                                    isPressed: compute(model.heldDirection, d => d === 3),
                                    disabled: model.noGuest, visibility: vis1,
                                    onClick: () => startDirectionWalk(model, 3),
                                }),
                                button({
                                    image: SPR_DIR_NE, width: "45px", height: "29px",
                            ...
                            horizontal([
                                button({
                                    image: SPR_DIR_SW, width: "45px", height: "29px",
                                    isPressed: compute(model.heldDirection, d => d === 2),
                                    disabled: model.noGuest, visibility: vis1,
                                    onClick: () => startDirectionWalk(model, 2),
                                }),
                                button({
                                    image: SPR_DIR_SE, width: "45px", height: "29px",
                ...
                toggle({
                    ...
                    width: "28px", height: "28px", border: false,
                    ...
                    onChange: (pressed: boolean) => {
                        ...
                        if (pressed) freezeGuest(model);
                        else unfreezeGuest(model);
                    },
                }),
                horizontal([
                    dropdown({
                        ...
                        disabled: model.noGuest, visibility: vis1,
                        onChange: (index: number) => { model.selectedActionIndex.set(index); },
                    }),
                    button({
                        text: "Perform", width: "60px", height: "16px",
            ...
            content: [
                horizontal([
                    toggle({
                        ...
                        width: "28px", height: "28px", border: false,
                        tooltip: "Play/Pause sequence",
                        disabled: model.noGuest,
                        isPressed: model.queuePaused,
                        visibility: vis2,
                        onChange: (pressed: boolean) => {
                            if (pressed) pauseQueue(model);
                            else resumeQueue(model);
                        },
                    }),
                    vertical({
                        width: "1w",
                        ...
                        content: [
                            checkbox({
                                ...
                                onChange: (checked: boolean) => {
                                    ...
                                    if (id !== null) {
                                        ...
                                        if (gs) {
                                            gs.keepSteps.set(!checked);
                                            if (checked) gs.loopQueue.set(false);
                                        }
                                    }
                                },
                            }),
                            checkbox({
                                ...
                                onChange: (checked: boolean) => {
                                    const id = model.selectedGuestId.get();
                                    if (id !== null) {
                                        guestStates.get(id)?.loopQueue.set(checked);
                                    }
                                },
                            }),
                        ],
                    ...
                    button({
                        text: "Clear", width: "70px", height: "28px",
                        disabled: compute(model.noGuest, model.queuePaused, (ng, paused) => ng || !paused),
                        visibility: vis2,
                        onClick: () => {
                            const cell = model.queueSelectedCell.get();
                            if (cell && cell.row >= 0) removeAction(model, cell.row);
                        },
                    }),
                    button({
                        text: "Clear All", width: "70px", height: "28px",
                        disabled: model.noGuest, visibility: vis2,
                        onClick: () => clearActions(model),
                    }),
                ]),
                listview({
                    height: "128px",
                    ...
                    onClick: (row: number) => {
                        ...
                        model.queueSelectedCell.set({ row, column: 0 });
                    },
                }),
                toggle({
                    text: "+ Move To", height: "20px",
                    isPressed: model.moveToolActive,
                    disabled: model.noGuest, visibility: vis2,
                    onChange: (pressed: boolean) => {
                        if (pressed) activateMoveTool(model);
                ...
                horizontal([
                    dropdown({
                        ...
                        disabled: model.noGuest, visibility: vis2,
                        onChange: (index: number) => { model.selectedQueueActionIndex.set(index); },
                    }),
                    label({ text: "for", width: "20px", visibility: vis2 }),
                    spinner({
                        width: "55px",
                    ...
                    button({
                        text: "+ Add", width: "60px", height: "18px",
                        disabled: model.noGuest, visibility: vis2,
                        onClick: () => {
